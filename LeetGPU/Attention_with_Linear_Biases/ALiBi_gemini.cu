...
__global__ void alibi_kernel(const float* Q, const float* K, const float* V, float* output, 
    ...
    */
    ...
    // 使用原子操作在 Shared Memory 中归约最大值
    // 注意：这是一个简化的实现，实际生产代码中应该使用更高效的归约算法（如树形归约）
    // 
    // 声明一个 Shared Memory 变量用于存储全局最大值
    ...
    // 原子操作：将每个线程的局部最大值与全局最大值比较并更新
    // atomicMax 只支持整数类型，所以需要将 float 转换为 int
    // __float_as_int 和 __int_as_float 是 CUDA 内置函数，用于浮点数和整数的位级转换
    // 注意：这种方法要求所有线程都参与，对于大数组可能成为性能瓶颈
    ...
    // 原子加法：将每个线程的局部和累加到全局分母
    // atomicAdd 支持 float 类型，可以直接使用
...
extern "C" void solve(const float* Q, const float* K, const float* V, float* output, 
    ...
    // shared_mem_size: 每个 Block 需要的 Shared Memory 大小（字节）
    // 用于存储该行的注意力分数 S[m][0..N-1]
    // 注意：N 不能太大，否则会超过每个 Block 的 Shared Memory 限制（通常为 48KB 或 96KB）
    // 例如：如果 N = 1024，则需要 1024 * 4 = 4096 字节（4KB），这在大多数 GPU 上都可以接受
